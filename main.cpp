...
#define PIN_WATTSINC    10
#define PIN_WATTSDEC     9
#define PIN_COIL_PWM    A3
...
const uint8_t DEBOUNCEWAITMS = 50; // ms to lock during debounce for
...
{
	if (getmV() < MINmV) // too small - makes the math bad
		return 0;
	if (getmV() > MAXmV) // more is merrier
		return 100;
	uint8_t value = ((getmV()-MINmV)*100)/(MAXmV-MINmV); // truncate as integer
	return value;
}
...
void updateline3()
{
	if (getmV() < MINmV) { // charge the damn batteries
...
void PWM_coil()
{
	analogWrite(PIN_COIL_PWM,(MW * 255) / (getmV() * getmV() / COILmOHMS)); // percentage of 0-255 .. returns badly if getmV() is wrong
}
...
void setup()
{
	...
	u8g.firstPage();
	do {
		if (getmV() > MINmV) // test if low battery
